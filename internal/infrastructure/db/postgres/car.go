...
func (self *CarRepository) GetCarByID(ctx context.Context, id entity.CarID) (*entity.Car, error) {
	var car dto.CarDTO
	sql, args := sq.Select("id", "brand", "model", "year", "color", "vin", "license_plate_number", "registration_certificate").
		From("cars").
		Where(sq.Eq{"id": id}).
...
func (self *CarRepository) CreateCar(ctx context.Context, car *entity2.Car) error {
	_, err := sq.Insert("cars").
		Columns("id", "brand", "model", "year", "color", "vin", "license_plate_number", "registration_certificate").
		Values(car.ID, car.Brand, car.Model, car.Year, car.Color, car.VIN, car.LicensePlateNumber, car.RegistrationCertificate).
